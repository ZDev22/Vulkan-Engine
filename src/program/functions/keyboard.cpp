...
Keyboard::Keyboard(AppWindow& appWindow) : window{appWindow.getWindow()}{
    glfwKeys = {
        // Special characters
    ...
    // Build fast lookup table: key code -> array index
    for (int i = 0; i < glfwKeys.size(); ++i) {
        keyIndexMap[glfwKeys[i]] = i;
    }
}
...
bool Keyboard::keyHit(int key) {
    auto it = keyIndexMap.find(key);
    if (it == keyIndexMap.end()) return false;
    if (glfwGetKey(window, key) == GLFW_PRESS) {
        if (keys[it->second] == 0) {
            keys[it->second] = 1;
            return true;
        } else if (keys[it->second] == 1) {
            keys[it->second] = 2;
        }
        return false;
    }
